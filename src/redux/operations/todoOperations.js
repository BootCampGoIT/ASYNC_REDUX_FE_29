...
export const postTasksOperation = (task) => async (dispatch) => {
    try {
        dispatch(loaderOn());
        dispatch(resetError());
        const response = await axios.post('https://ebc-29-async-redux.firebaseio.com/tasks.json', task);
        (response.data.status === 200) && dispatch(addNewTask({ id: response.data.name, ...task }))
